 ...
 * @since v1.0
 */
public class XddfLineChartBuilder extends AbstractMultiXddfChartBuilder {

    private boolean isSmooth = false;

    public XddfLineChartBuilder() {
        super(ChartTypes.LINE);
    }

    @Override
    public void configSeriesTitle(XSSFChart chart, XDDFChartData data, String title, int chartIndex, int index) {
        if (chart.getCTChart().getPlotArea().getLineChartArray(chartIndex).getSerArray(index).getTx() ==
                null) {
            chart.getCTChart().getPlotArea().getLineChartArray(chartIndex).getSerArray(index).addNewTx();
            chart.getCTChart().getPlotArea().getLineChartArray(chartIndex).getSerArray(index).addNewSmooth().setVal(isSmooth);
        }
    ...
    public void configChartFeature(XSSFChart chart, XDDFChartData data, int chartIndex) {
        chart.getCTChart().getPlotArea().getLineChartArray(chartIndex).addNewAxId().setVal(chart.getAxis().get(0).getId());
        chart.getCTChart().getPlotArea().getLineChartArray(chartIndex).addNewAxId().setVal(chart.getAxis().get(1).getId());
    }
...
//        list.add(new RowSerieConfig("数据6", 6, 0, 9));
        XddfLineChartBuilder builder = new XddfLineChartBuilder();
        ...
        try (FileOutputStream fileOut = new FileOutputStream("ooxml-LINE-chart2.xlsx")) {
            wb.write(fileOut);
        }
    }
